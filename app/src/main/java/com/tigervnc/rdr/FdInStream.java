...
public class FdInStream extends InStream {
  ...
  // 🔑 Instrumentation: select can spin several times inside the loop, so it is attributed to the
  //    one read.
  //    🔴 This axis is what established that select is **waiting**, not overhead (a still screen
  //    gave sel = 998ms/s).
  private long selAccum = 0;
  ...
  protected int overrun(int itemSize, int nItems, boolean wait)
  {
    ...
    while (end < itemSize) {
      ...
      if (!timing) {
        ...
        // Raising this to an 8KB floor was tried on 2026-09-16 and made **no difference**: read
        //    moved −1.2%, inside the noise. The number of socket reads is not the bottleneck —
        //    90% of the read stretch is select, waiting for data.
  ...
  protected int readWithTimeoutOrCallback(ByteBuffer buf, int len, boolean wait) {
    ...
    long trd0 = System.nanoTime();
    try {
      n = fd.read(buf, len);
    } catch (Exception e) {
      throw new SystemException("read:"+e.toString());
    }
    io.github.zirize.vncviewerforgames.perf.PerfStats.INSTANCE.sockRead(
        selAccum, System.nanoTime() - trd0);
    selAccum = 0;
